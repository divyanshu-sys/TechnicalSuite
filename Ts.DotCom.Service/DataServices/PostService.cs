...
using Ts.DotCom.Domain.HelperModels;
...
using Ts.DotCom.Service.PostHelpers;
using Ts.Dto;
    ...
    public class PostService : IPostService
    {
        ...
        public async Task<ResponseMessageDto<PostDto>> CreateAsync(CreatePostDto modelDto, string userId)
        {
            ...
            catch (DbUpdateException ex)
            {
                ...
                logger.LogError(ex, "Create Post failed: {Message}", msg);
        ...
        public async Task<ResponseMessageDto<bool>> PublishPostAsync(int postId, PublishPostDto modelDto, string userId, bool isAdmin)
        {
            ...
            {
                responseResult.ErrorMessage.Add("Unauthorized to publish this post.");
                return responseResult;
            }
        ...
        public async Task<IEnumerable<GetPostForListViewDto>> GetForListViewAsync(PostDataTableForViewRequestDto modelDto)
        {
            ...
            foreach (var entity in entities)
            {
                var dto = new GetPostForListViewDto
                {
                    ...
                    MainImageUrl = entity.MainImage == null ? null : $"{config.GetValue<string>("SrcApiDotCom:PostImageUrl")}/{entity.MainImage}",
        ...
        public async Task<string> GetSitemapAsync()
        {
            ...
            foreach (var entity in entities)
            {
                sitemapData.Urls.Add(new UrlData
                {
                    ...
                    Loc = $"{config.GetValue<string>("SrcApiDotCom:SiteUrl")}/{categories.FirstOrDefault(x => x.Id == entity.CategoryId)?.Name}/{subCategories.FirstOrDefault(x => x.Id == entity.SubCategoryId)?.Name}/{entity.PostLink}".ToLower()
                });
            }
            sitemapData.Urls.Add(new UrlData
            {
                LastMod = DateTime.UtcNow.ToDateTimeUtcString(),
                Loc = config.GetValue<string>("SrcApiDotCom:SiteUrl")
            });
