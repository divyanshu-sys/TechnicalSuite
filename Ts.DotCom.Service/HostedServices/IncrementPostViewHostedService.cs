...
namespace Ts.DotCom.Service.HostedServices
{
    ...
    public class IncrementPostViewHostedService : BackgroundService
    {
        ...
        {
            logger.LogInformation("DotCom - IncrementPostView Hosted Service running.");
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                if (!postQueue.IsEmpty)
                {
                    ...
                    logger.LogInformation($"DotCom - Database Processing: Incremented total views.");
                }
                await Task.Delay(TimeSpan.FromMinutes(2.01), cancellationToken).ConfigureAwait(false);
            }

            logger.LogInformation("DotCom - IncrementPostView Hosted Service stopping.");
        }
        ...
        private async Task ProcessWorkAsync(IEnumerable<int> ids)
        {
            ...
            {
                logger.LogError(ex, "DotCom - Error while incrementing the post view.");
            }
        }
    }
}
