...
{
    public class HttpClientUserService : IHttpClientUserService
    {
        ...
        public HttpClientUserService(IConfiguration configuration,
        ...
        private async Task<bool> ConfigureAuthHeaderAsync()
        {
            ...
            if (tokenHandler.ReadToken(apiToken) is JwtSecurityToken payload && payload.ValidTo < DateTime.UtcNow.AddSeconds(30)) // Token has expired
            {
                ...
                var httpResponseMessage = await httpClient.PostAsync($"{ApiBaseUri}/{ApiUrl}/authclientusershopin/refresh-token",
