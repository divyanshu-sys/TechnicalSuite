...
namespace Ts.ShopIn.Client.HttpClientServices.ClientServices
{
    public class ContactClient : IContactClient
    {
        ...
        public async Task<ResponseMessageDto<bool>> ContactAsync(ContactFormVm model)
        {
            ...
            var response = await httpPublicService.PostAsync<bool>($"{BaseUrl}/{ApiUrl}/shopinapp", modelDto, true).ConfigureAwait(false);
            if (response.ErrorMessage.Count > 0)
                logger.LogError("Contact form submission failed with errors: {ErrorMessages}", string.Join(',', response.ErrorMessage));
            return response;
        }
    }
}
