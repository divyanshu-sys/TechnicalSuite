...
{
    [AllowAnonymous]
    [RazorValidateGoogleReCaptcha(GoogleCaptchaConstant.GoogleCaptchaAdminAppSiteKey,
    ...
    public class IndexModel : PageModel
    {
        ...
        public IndexModel(IApplicationUserClient applicationUserClient, IBrowserDetector browserDetector)
        {
        ...
        public async Task<IActionResult> OnPost()
        {
            ...
            var response = await applicationUserClient.LoginAsync(Model).ConfigureAwait(false);
            ...
            if (tokenHandler.ReadToken(response.Data.ApiToken) is JwtSecurityToken payload)
            {
                ...
                TempData["notify"] = "Welcome to Technical Suite.";
            }
            if (!string.IsNullOrEmpty(ReturnUrl) && Url.IsLocalUrl(ReturnUrl))
                return Redirect(ReturnUrl);
            return RedirectToPage("/index");
