...
namespace Ts.WebApi.Middlewares
{
            ...
            catch (Exception ex)
            {
                ...
                if (config.GetValue<bool>("IsSendExceptionMail"))
                {
                    var emailData = new EmailData
                    {
                        ...
                        Subject = "Unhandled Error - Web Api",
                        IsBodyHtml = true,
                ...
                if (webHostEnvironment.IsDevelopment())
                    ...
                    await context.Response.WriteAsync($"{(ex.InnerException == null ? ex.Message : ex.InnerException.Message)}\nPlease contact developer.").ConfigureAwait(false);
            }
        }
    }
}
