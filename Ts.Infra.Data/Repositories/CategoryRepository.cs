...
namespace Ts.Infra.Data.Repositories
{
    public class CategoryRepository : GenericRepository<Category>, ICategoryRepository
    {
        ...
        public CategoryRepository(TsIdentityDbContext dbContext) : base(dbContext)
        {
            this.dbContext = dbContext;
        }

        public Task<List<Category>> GetAllAsync(CategoryDataTableRequest requestModel)
        {
            var queriableEntity = CommonSearch(requestModel);

            // Ordering
            IOrderedQueryable<Category> orderQueriableEntity = null;
        ...
        {
            var queriableEntity = dbContext.Categories.AsQueryable();
